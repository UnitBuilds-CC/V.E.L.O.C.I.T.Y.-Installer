//! Test: use compiler's build_msi() with test's data
//! This tells us if the issue is in the compiler's code path or its data
use velocity_msi::{MsiBuilder, Column, Value, CabinetFile, build_cabinet};
use std::process::Command;

fn main() {
    println!("=== Test: compiler code path with test data ===\n");

    // Build MSI using the EXACT same data as test_replicate_exact.rs
    // but going through a code path similar to the compiler
    ...
    // Create tables EXACTLY as compiler does (in same order)
    create_msi_tables(&mut b);

    // Populate with EXACT same data as test_replicate_exact.rs
    populate_test_data(&mut b);

    // Build cabinet (using velocity_msi like test)
    let file_names: Vec<&str> = vec![
        "sample-app.exe", "core.dll", "version.txt",
    ...
    if code == 1620 {
        println!("FAIL: 1620 = invalid package");
        println!("The compiler's CODE PATH is broken (not the data)");
    } else if code == 0 {
        println!("SUCCESS: MSI installed!");
    } else {
        println!("Valid MSI (error {} = install issue, not package issue)", code);
    }

    // Check log
    if let Ok(content) = std::fs::read_to_string(log) {
        for line in content.lines() {
            if line.contains("Product Name") || line.contains("1620") || line.contains("installed the product") {
...
fn create_msi_tables(b: &mut MsiBuilder) {
    // Create tables in EXACT same order as compiler's create_msi_tables()
...
fn populate_test_data(b: &mut MsiBuilder) {
    ...
    // InstallExecuteSequence (standard actions only, like test)
